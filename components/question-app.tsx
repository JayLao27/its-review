...
function stringifyValue(value: unknown): string {
  ...
  if (typeof value === "object") return Object.entries(value as Record<string, unknown>).map(([k, v]) => `${k} ${stringifyValue(v)}`).join(" ");
  return String(value);
}
...
export function QuestionApp({ questions }: QuestionAppProps) {
  ...
  const reviewedCount = filteredQuestions.filter((q) => revealedIds.has(q.id)).length;
  const progressPercent = filteredQuestions.length > 0 ? Math.round((reviewedCount / filteredQuestions.length) * 100) : 0;
  ...
  const currentDragMapping = currentQuestion ? dragMappingsByQuestion[currentQuestion.id] ?? {} : {};
  const currentDropTargets = currentQuestion ? dropTargetsByQuestion[currentQuestion.id] ?? [] : [];
  ...
  useEffect(() => {
    ...
    if ((currentQuestion as any).type === "drag_and_drop") {
      ...
      setDropTargetsByQuestion((current) => {
        ...
        const shuffled = targets.map((t) => ({ t, r: Math.random() })).sort((a, b) => a.r - b.r).map((x) => x.t);
  ...
  useEffect(() => {
    if (currentQuestion && (currentQuestion as any).type === "drag_and_drop" && hasAnsweredCurrentQuestion) {
      revealCurrent(currentQuestion.id);
    }
      ...
      return (
        <div className="space-y-4 px-4 py-4 sm:px-5">
            ...
            <div>
              ...
              <div className="space-y-2">
                {currentDropTargets.map((target) => {
                  ...
                  const correctKey = Object.entries((currentQuestion as any).answer ?? {}).find(([, v]) => v === target)?.[0] ?? null;
                  const isWrongAssignment = revealedIds.has(currentQuestion.id) && assignedKey && assignedKey !== correctKey;
                  const showMismatchMessage = revealedIds.has(currentQuestion.id) || reviewMode;
          ...
          <div className="mt-4 space-y-3 rounded-none border border-border bg-card/60 p-4">
            {revealedIds.has(currentQuestion.id) && currentQuestion.explanation ? (
              <div className="mt-4 rounded-none border-l-4 border-primary/60 bg-primary/5 px-4 py-3 text-sm text-primary/90">
                <div className="text-[10px] uppercase tracking-[0.18em] text-primary mb-1">Explanation</div>
      ...
      return (
        <div className="space-y-3 px-4 py-4 sm:px-5">
          ...
          {currentQuestion.options.map((opt, idx) => {
            ...
            const isCorrect = revealedIds.has(currentQuestion.id) && isCorrectAnswer;
            const isWrong = revealedIds.has(currentQuestion.id) && isSelected && !isCorrectAnswer;
          ...
          <CardContent className="space-y-4 p-4 sm:p-5">
            ...
            <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex-1 min-w-[200px]">
                <div className="mb-2 flex items-center justify-between text-[10px] uppercase tracking-[0.22em] text-muted-foreground"><span>Triage coverage</span><span>{progressPercent}%</span></div>
        ...
        <div>
          <div className="grid gap-2 sm:grid-cols-[1fr_auto] mb-2">
            <div className="relative">
              ...
              <Input className="h-12 rounded-none border-border bg-card/80 pl-10 text-foreground placeholder:text-muted-foreground focus-visible:ring-ring" placeholder="Search findings, topics, or response text" value={query} onChange={(e) => setQuery(e.target.value)} />
            </div>
            <Button type="button" variant="outline" className="h-12 rounded-none border-border bg-card/60 text-[12px] font-semibold uppercase tracking-[0.18em] text-foreground" onClick={() => setQuery("")}>Clear</Button>
          </div>
